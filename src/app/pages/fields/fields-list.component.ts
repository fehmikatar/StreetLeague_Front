    ...
    <div class="min-h-screen bg-background p-4 md:p-6">
      <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8">
          <div>
            <h1 class="mb-2">My Fields</h1>
            <p class="text-muted-foreground">Manage your sports spaces</p>
          </div>
          <a routerLink="/app/fields/add" class="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-xl hover:bg-primary/90 transition-all shadow-lg shadow-primary/30">
            <lucide-icon [name]="PlusIcon" [size]="16"></lucide-icon>Add a field
          </a>
        </div>
        ...
        <div *ngIf="!loading && fields.length === 0" class="text-center py-20 text-muted-foreground">
          ...
          <p class="font-semibold mb-2">No fields for the moment</p>
          <a routerLink="/app/fields/add" class="text-primary hover:underline text-sm">Add your first field →</a>
        </div>
  ...
  deleteField(field: any) {
    ...
    this.http.delete(`${environment.apiUrl}/sport-spaces/${field.id}`).subscribe({
      next: () => {
        ...
        this.showNotification(`✅ Field "${field.name}" deleted`);
      },
