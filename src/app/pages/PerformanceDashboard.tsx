...
export default function PerformanceDashboard() {
  ...
  const addXpMutation = useMutation({
    ...
    onSuccess: () => {
      setWorkflowStep(1);
      toast.success('XP vérifié et enregistré !');
    },
  ...
  const awardBadgeMutation = useMutation({
    ...
    onError: (error: any) => toast.error(error.message || 'Le joueur possède déjà ce badge ou une erreur est survenue.')
  });
  ...
  const generatePromoMutation = useMutation({
    ...
    onSuccess: () => {
      setWorkflowStep(3);
      toast.success('Code promo généré et sauvegardé !');
    },
    onError: (error: any) => toast.error(error.message || 'Erreur lors de la génération du code promo.')
  });
  ...
  const handleAwardBadge = () => {
    if (!selectedPerformance || !badges || badges.length === 0) {
      toast.warning("Aucun badge disponible dans le système.");
      return;
    }
  ...
  const handleGeneratePromo = () => {
    ...
    generatePromoMutation.mutate({
      name: `Récompense Badge ${awardedBadge.name}`,
      promoCode: code,
      discount: 20,
  ...
  const handleApplyPromo = () => {
    toast.success(`Code promo ${generatedPromo} appliqué avec succès ! La réduction est validée.`);
    setWorkflowStep(4);
    ...
    <div className="container mx-auto py-10 px-4 max-w-7xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-2">Gérez les performances et déclenchez le workflow de récompense.</p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid gap-3">
                    <Label className="font-medium">Buts marqués <span className="text-muted-foreground font-normal text-xs">(0-20)</span></Label>
                    <Input 
                  ...
                  <div className="grid gap-3">
                    <Label className="font-medium">Passes décisives <span className="text-muted-foreground font-normal text-xs">(0-15)</span></Label>
                    <Input 
                      className="bg-background text-lg font-semibold" 
                ...
                <div className="md:col-span-2 p-5 bg-gradient-to-r from-primary/10 to-accent/10 rounded-xl border border-primary/20">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Évaluation de la performance sur 10</p>
                    </div>
                    <div className="w-1/3">
                      <Input 
        ...
        <DialogContent className="sm:max-w-[550px] bg-card border-border/50 text-foreground shadow-2xl rounded-2xl overflow-hidden p-0">
          <DialogHeader className="bg-gradient-to-r from-primary/10 to-accent/10 px-6 py-6 border-b border-border/50">
            <DialogTitle className="text-2xl font-bold flex items-center gap-3 text-foreground">
              ...
              Workflow de Récompense
            </DialogTitle>
            <p className="text-sm text-muted-foreground mt-2">
              Suivez les étapes pour attribuer les récompenses suite à cette performance.
            </p>
          </DialogHeader>
          <div className="p-6 space-y-4 bg-background/50">
            ...
            <div className={`p-5 border rounded-xl flex items-center justify-between transition-all duration-300 ${workflowStep >= 0 ? 'bg-card border-primary/40 shadow-sm ring-1 ring-primary/10' : 'opacity-60 grayscale bg-muted/50 border-border'}`}>
              <div className="pr-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="font-semibold text-foreground text-lg">Détection de niveau</h3>
                </div>
                <p className="text-sm text-muted-foreground pl-8">Calcule l'XP pour cette performance.</p>
                {workflowStep >= 1 && <span className="text-sm font-semibold text-primary mt-2 ml-8 inline-block bg-primary/10 px-2 py-1 rounded-md">+{calculatedXp} XP confirmés !</span>}
              </div>
              <div>
                {workflowStep === 0 ? (
                  <Button onClick={handleVerifyXp} disabled={addXpMutation.isPending} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-md transition-all hover:scale-105">
                    {addXpMutation.isPending ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : 'Vérifier XP'}
                  </Button>
            ...
            <div className={`p-5 border rounded-xl flex items-center justify-between transition-all duration-300 ${workflowStep >= 1 ? 'bg-card border-primary/40 shadow-sm ring-1 ring-primary/10' : 'opacity-60 grayscale bg-muted/50 border-border'}`}>
              <div className="pr-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="font-semibold text-foreground text-lg">Récompense de Badge</h3>
                </div>
            ...
            <div className={`p-5 border rounded-xl flex items-center justify-between transition-all duration-300 ${workflowStep >= 2 ? 'bg-card border-primary/40 shadow-sm ring-1 ring-primary/10' : 'opacity-60 grayscale bg-muted/50 border-border'}`}>
              <div className="pr-4">
                ...
                <p className="text-sm text-muted-foreground pl-8">Génère un code de réduction temporaire.</p>
              ...
              <div>
                {workflowStep === 2 ? (
                  <Button onClick={handleGeneratePromo} disabled={generatePromoMutation.isPending} className="bg-yellow-500 hover:bg-yellow-600 text-white shadow-md transition-all hover:scale-105">
                    {generatePromoMutation.isPending ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : 'Générer Promo'}
                  </Button>
            ...
            {workflowStep >= 3 && (
               <div className="p-6 mt-6 border-2 rounded-xl bg-gradient-to-br from-green-500/10 to-emerald-500/5 border-green-500/30 flex flex-col items-center justify-center space-y-5 animate-in slide-in-from-bottom-4 fade-in duration-500 shadow-inner">
                 <div className="text-center space-y-2">
                   ...
                   <h3 className="font-extrabold text-green-600 dark:text-green-400 text-xl">Félicitations !</h3>
                   <p className="text-sm text-muted-foreground">
                     Utilisez le code <strong className="font-mono text-foreground bg-muted px-2 py-1 rounded border border-border shadow-sm text-base mx-1">{generatedPromo}</strong> pour profiter de 20% de réduction.
                   </p>
                 </div>
